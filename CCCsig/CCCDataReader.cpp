...
CCCDataReader::CCCDataReader(string file) {
	...
	for(map<ChromosomeIndexType, set<Segment> >::iterator it = segs.begin(); it != segs.end(); it++) {
		...
		contactMatrices.insert(pair<ChromosomeIndexType, CCCMatrixInteraction >(it->first, mat));
...
void CCCDataReader::buildContactMatrices() {
	...
	while (inFile >> chrL >> startL >> endL >> chrR >> startR >> endR >> nij) {
		...
		Interaction interaction(nij,0,1.0);
		contactMatrices[nChrL].setElement(segL, segR, interaction);
	}
	inFile.close();
